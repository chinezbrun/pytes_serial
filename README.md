...
These options can be activated/deactivated in the configuration file (`pytes_serial.cfg`).  

When MQTT transmission is activated:
- The JSON file is sent as payload to the following topic:  
  `homeassistant/sensor/pytes/state`
- The program has built-in integration with Home Assistant, where the following sensors are automatically created for each battery: `current`, `voltage`, `temperature`, `soc`, `status`.  
  The battery number is appended at the end of each sensor (i.e. `current_1`, `current_2...`).  

When cell monitoring is activated, an additional device will be created in Home Assistant with the suffix `_cells`, with all associated sensors. The battery and cell number are appended at the end of each sensor (i.e. `voltage_102` means voltage for battery 1 cell 2).  
Basic statistics are also implemented. Therefore, additional sensors will be available for min, max, and delta for cell voltage and temperature.  
(i.e. `pytes_cells_voltage_max_1` means max cell voltage for battery 1).  
...
2. Optional:  
   a. If you want to use MariaDB:  
      - MariaDB must be installed (MariaDB documentation is out of this project scope)  
   ...
   b. If you want to use MQTT / MQTT integration in Home Assistant:  
      - An MQTT broker must be installed (MQTT documentation is out of this project scope)  
